...
V1 런타임의 authoritative state는 `common-employee/runtime/autonomous-runtime.db`에 두고,
`docs/status/*`와 `docs/generated/decision-logs/*`는 이 상태를 사람이 읽는 형태로 동기화한 산출물로 둔다.
로컬 웹 콘솔은 이 런타임 위에 얇게 올라가 mock/manual intake를 제출하고,
최근 run, gate 결과, generated artifact를 브라우저에서 확인하는 읽기/쓰기 진입점이다.
...
└────────────────────────────┼────────────────────────────────┘
                             ...
                             ▼
┌──────────────────────────────────────────────────────────────┐
│             로컬 운영 UI / Runtime Surface                      │
│                                                              │
│   ┌────────────────────────────────────────────────────┐    │
│   │          Local Web Console + CLI Entrypoints         │    │
│   │   manual/mock intake · recent runs · artifact view   │    │
│   └────────────────────────┬───────────────────────────┘    │
└────────────────────────────┼────────────────────────────────┘
                             │
                             ▼
...
[Lead / Coordinator] task brief 기준 고정
  │
  ▼
[Runtime DB] authoritative state + attempts + gate verdict
  │
  ├─ [Local Web Console] recent run list / run detail / artifact view
  │
  ▼
...
- 런타임 상태 저장소: `runtime/autonomous-runtime.db`
