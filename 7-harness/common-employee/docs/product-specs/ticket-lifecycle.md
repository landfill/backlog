...
- Teams 알림 (`docs/integrations/teams.md`)
...
- Jira 댓글로 처리 결과 공유
- Teams로 유관자 알림 (`docs/integrations/teams.md`)
- Confluence 보고서 업데이트 (프로젝트성인 경우)

**운영 상태:**
- Reporter는 의사결정 로그와 외부 공유 산출물을 갱신한다
- tracker 변경이 필요하면 Lead 또는 Coordinator가 verdict, evidence, next action을 최신 상태로 갱신한다

**산출물**: 의사결정 로그 파일 + 유관자 알림 + 티켓 댓글
